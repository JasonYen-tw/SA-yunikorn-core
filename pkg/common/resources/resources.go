...
// ---------------------------------------------------------------------------
// 🆕  Helper functions – CPU / Memory fast-path
// ---------------------------------------------------------------------------

// 返回 CPU (vcore) 整數值；若 nil 或未定義則 0
func (r *Resource) GetCPU() int64 {
	if r == nil {
		return 0
	}
	return int64(r.Resources[common.CPU])
}

// 返回 Memory (bytes) 整數值；若 nil 或未定義則 0
func (r *Resource) GetMemory() int64 {
	if r == nil {
		return 0
	}
	return int64(r.Resources[common.Memory])
}

// 方便測試或臨時計算用：直接用 cpu / mem 建立一個 Resource
func NewResourceFromCPUAndMem(cpuVCore, memBytes int64) *Resource {
	res := NewResource()
	if cpuVCore != 0 {
		res.Resources[common.CPU] = Quantity(cpuVCore)
	}
	if memBytes != 0 {
		res.Resources[common.Memory] = Quantity(memBytes)
	}
	return res
}
