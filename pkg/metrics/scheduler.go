...
type SchedulerMetrics struct {
	...
	allocationThroughput        *prometheus.CounterVec
	...
	schedulingLatencyByStrategy *prometheus.HistogramVec
	...
	saSuggestionOutcome         *prometheus.CounterVec
	pendingAsks                 *prometheus.GaugeVec
	mainLoopTime                *prometheus.CounterVec
	lock                        locking.RWMutex
}

const (
	StrategyNative    = "native"
	StrategyAnnealing = "annealing"

	OutcomeSuccess = "success"
	OutcomeFailure = "failure"

	SAOutcomeAttempt  = "attempt"
	SAOutcomeAdopted  = "adopted"
	SAOutcomeRejected = "rejected"

	MainLoopStateExecute = "execute"
	MainLoopStateIdle    = "idle"
	MainLoopStateBlocked = "blocked"
)
...
func InitSchedulerMetrics() *SchedulerMetrics {
	...
	s.saSuggestionOutcome = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: Namespace,
			Subsystem: SchedulerSubsystem,
			Name:      "sa_suggestion_total",
			Help:      "Count of SA suggestions by outcome (attempt, adopted, rejected)",
	...
	s.allocationThroughput = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: Namespace,
			Subsystem: SchedulerSubsystem,
			Name:      "allocation_total",
			Help:      "Successful allocation decisions per partition and strategy.",
		}, []string{"partition", "strategy", "outcome"})
	...
	s.schedulingLatencyByStrategy = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Namespace: Namespace,
			Subsystem: SchedulerSubsystem,
			Name:      "scheduling_latency_seconds",
			Help:      "Latency of scheduling results partitioned by strategy.",
	...
	s.pendingAsks = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Namespace: Namespace,
			Subsystem: SchedulerSubsystem,
			Name:      "pending_asks",
			Help:      "Number of pending allocation asks per partition.",
		}, []string{"partition"})
	s.mainLoopTime = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: Namespace,
			Subsystem: SchedulerSubsystem,
			Name:      "mainloop_state_seconds_total",
			Help:      "Time spent in scheduler main loop states.",
		}, []string{"partition", "state"})

	// Register the metrics
	var metricsList = []prometheus.Collector{
		s.containerAllocation,
		s.allocationThroughput,
		s.applicationSubmission,
		s.application,
		s.node,
		s.schedulingLatency,
		s.schedulingLatencyByStrategy,
		s.sortingLatency,
		s.tryNodeLatency,
		s.tryPreemptionLatency,
		s.saIterations, s.saLatency, s.saBestCost,
		s.saFailures,
		s.saRunning, s.saPendingDecisions, s.saLastResultSize, s.saSuggestionOutcome,
		s.pendingAsks, s.mainLoopTime,
	}
...
func (m *SchedulerMetrics) Reset() {
	...
	m.saSuggestionOutcome.Reset()
	m.schedulingLatencyByStrategy.Reset()
	m.pendingAsks.Reset()
	m.mainLoopTime.Reset()
}
...
func (m *SchedulerMetrics) RecordSASuggestionAttempt(partition string) {
	m.saSuggestionOutcome.WithLabelValues(partition, SAOutcomeAttempt).Inc()
}

func (m *SchedulerMetrics) RecordSASuggestionResult(partition string, adopted bool) {
	outcome := SAOutcomeRejected
	if adopted {
		outcome = SAOutcomeAdopted
	}
	m.saSuggestionOutcome.WithLabelValues(partition, outcome).Inc()
}

func (m *SchedulerMetrics) ObserveAllocationThroughput(partition, strategy string, success bool) {
	...
	m.allocationThroughput.WithLabelValues(partition, strategy, outcome).Inc()
}

func (m *SchedulerMetrics) SetPendingAskCount(partition string, count int) {
	m.pendingAsks.WithLabelValues(partition).Set(float64(count))
}
...
func (m *SchedulerMetrics) ObserveSchedulingLatencyByStrategy(partition, strategy string, start time.Time) {
	if strategy == "" {
		strategy = StrategyNative
	}
	m.schedulingLatencyByStrategy.WithLabelValues(partition, strategy).Observe(time.Since(start).Seconds())
}

func (m *SchedulerMetrics) AddMainLoopDuration(partition, state string, dur time.Duration) {
	if state == "" {
		state = MainLoopStateIdle
	}
	m.mainLoopTime.WithLabelValues(partition, state).Add(dur.Seconds())
}
