...
func (cc *ClusterContext) notifyRMNewAllocation(rmID string, alloc *objects.Allocation) {
	// [FIXED] 在單元測試中，rmEventHandler 可能為 nil。添加一個保護性檢查。
	if cc.rmEventHandler == nil {
		log.Log(log.SchedContext).Debug("skip notifying RM, no event handler available (running in test?)",
