...
type PartitionContext struct {
	...
	Strategy               string
	SAInstance             strategy.SchedulerStrategy
	SABuffer               []*objects.AllocationResult // SA 產生後暫存的結果
	saIndex                int                         // 下一筆要 pop 的位置
...
// 背景執行 SA
func (pc *PartitionContext) runSAInBackground() {
	for {
		// 1. 將耗時的 Schedule 操作放在鎖外，它現在會返回一個決策列表
		results := pc.SAInstance.Schedule(pc)
